...
//         bytes memory empty;
//         bytes memory blueCar = abi.encodePacked("blue");
//         uint40 redCar = uint40(bytes5(bytes.concat("255", "0", "0")));
//         bytes memory yellowCar = abi.encode(type(uint256).min, type(uint256).min);
//         bytes memory carousel = abi.encodeWithSignature("_BumperCars(bytes,uint40,bytes)", blueCar, redCar, yellowCar);
...
//         console.log(blueCar.length + 5);
//         console.log(yellowCar.length);
//         (uint256 x, bytes memory _rainbowCar) = abi.decode(crash, (uint256, bytes));
//         // console.log("after");
//         // console.log(_rainbowCar.length);

//         // nextRide = abi.encodePacked(_rainbowCar);
//         // console.log("nextRide", nextRide.length);
